...
public class BandPassFilter extends Filter {
    public BandPassFilter(int size, int samplingRate, double hpFc, int hpOrder, FilterTypeEnum hpFilterType, double lpFc, int lpOrder, FilterTypeEnum lpFilterType) {
        this.size = size;
        this.hpFc = 2.0*hpFc/samplingRate;;
        this.hpOrder = hpOrder;
        this.hpFilterType = hpFilterType;
    ...
    public double[] getImpulse() {
        ...
        /*for(int i = 0; i < (n/2)+1; ++i) {
            double nFreq = 2.0*(double)i/(double)n/(double)hpFc;
            mag[i] = 1 - Math.pow((1.0 + Math.pow(nFreq,2.0*this.hpOrder)),-1.0);
            nFreq = 2.0*(double)i/(double)n/(double)lpFc;
            mag[i] *= Math.pow((1.0 + Math.pow(nFreq,2.0*this.lpOrder)),-1.0);
        }*/

        switch(this.hpFilterType) {
            case BUTTERWORTH:
                for(int i = 0; i < (n/2)+1; ++i) {
                    double nFreq = 2.0*(double)i/(double)n/(double)hpFc;
                    mag[i] = Math.sqrt(1.0-1.0/(1.0 + Math.pow(nFreq,2.0*this.hpOrder)));
                    nFreq = 2.0*(double)i/(double)n/(double)lpFc;
            ...
            case LINKWITZ_RILEY:
                for(int i = 0; i < (n/2)+1; ++i) {
                    double nFreq = 2.0*(double)i/(double)n/(double)hpFc;
                    mag[i] = 1.0-1.0/(1.0 + Math.pow(nFreq,2.0*this.hpOrder));
                    nFreq = 2.0*(double)i/(double)n/(double)lpFc;
